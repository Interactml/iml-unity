...
namespace InteractML
{
    ...
    public class SeriesTrainingExamplesNodeEditor : NodeEditor
    {
        #region Variables

        private TrainingExamplesNode m_SeriesTrainingExamplesNode;
        ...
        public override void OnBodyGUI()
        {
            ...
            if (m_SeriesTrainingExamplesNode.EnableKeyboardControl)
            {
                GUILayout.Box("<b>Keyboard Shortcuts:</b>\n <b>Record:</b> " + m_SeriesTrainingExamplesNode.RecordDataKey.ToString(),
                    styleGUIBox);

                // Show key configs
                m_SeriesTrainingExamplesNode.RecordDataKey = (KeyCode)EditorGUILayout.EnumFlagsField(m_SeriesTrainingExamplesNode.RecordDataKey);
        ...
        private void ShowTotalNumberOfTrainingData()
        {
            ...
            switch (m_SeriesTrainingExamplesNode.ModeOfCollection)
            {
                case TrainingExamplesNode.CollectionMode.SingleExample:
                    numberExamplesText = m_SeriesTrainingExamplesNode.TotalNumberOfTrainingExamples.ToString();
                    EditorGUILayout.LabelField("Total No. Training Examples: ", numberExamplesText);
                    EditorGUILayout.LabelField("Total No. Desired Outputs: ", m_SeriesTrainingExamplesNode.DesiredOutputFeatures.Count.ToString());
                    break;
                case TrainingExamplesNode.CollectionMode.Series:
                    numberExamplesText = m_SeriesTrainingExamplesNode.TrainingSeriesCollection.Count.ToString();
        ...
        private void ShowClearAllExamplesButton()
        {
            ...
            switch (m_SeriesTrainingExamplesNode.ModeOfCollection)
            {
                ...
                case TrainingExamplesNode.CollectionMode.Series:
                    ...
                    // Only run button logic when there are training examples to delete
                    if (!Lists.IsNullOrEmpty(ref m_SeriesTrainingExamplesNode.TrainingSeriesCollection))
                    {
                        disableButton = false;
                    }
                    break;
        ...
        private void ShowDesiredOutputFeaturesLogic ()
        {
            ...
            for (int i = 0; i < m_SeriesTrainingExamplesNode.DesiredOutputFeatures.Count; i++)
            {
        ...
        private void ShowDesiredInputsConfigLogic()
        {
            ...
            EditorGUILayout.LabelField("No. of Inputs ", m_SeriesTrainingExamplesNode.DesiredInputsConfig.Count.ToString());

            // Go through the list of inputs and show the correct kind of config editor tool
            for (int i = 0; i < m_SeriesTrainingExamplesNode.DesiredInputsConfig.Count; i++)
            {
                int inputValueIndex = i + 1;
                string label = "Input " + inputValueIndex;
                var inputFeature = m_SeriesTrainingExamplesNode.DesiredInputsConfig[i];
                // We make sure that the desired output feature list captures the value inputted by the user
                EditorGUILayout.EnumFlagsField(label, m_SeriesTrainingExamplesNode.DesiredInputsConfig[i]);
        ...
        private void ShowDesiredOutputsConfigLogic()
        {
            ...
            //EditorGUILayout.IntField("No. of Outputs ", m_SeriesTrainingExamplesNode.DesiredInputsConfig.Count.ToString());

            // Check if we are changing the size of the list 
            int originalSize = m_SeriesTrainingExamplesNode.DesiredOutputsConfig.Count;
            int newSize = EditorGUILayout.IntField("No. of Outputs", m_SeriesTrainingExamplesNode.DesiredOutputsConfig.Count);
            ...
            for (int i = 0; i < m_SeriesTrainingExamplesNode.DesiredOutputsConfig.Count; i++)
            {
                ...
                if (m_SeriesTrainingExamplesNode.DesiredInputsConfig.Count > 0 && i < m_SeriesTrainingExamplesNode.DesiredInputsConfig.Count)
                {
                    var inputFeature = m_SeriesTrainingExamplesNode.DesiredInputsConfig[i];
                    // We make sure that the desired output feature list captures the value inputted by the user
                    EditorGUILayout.EnumFlagsField(label, m_SeriesTrainingExamplesNode.DesiredOutputsConfig[i]);
                }
        ...
        private void ShowTrainingExamplesDropdown()
        {
            ...
            if (m_ShowTrainingDataDropdown)
            {
                ...
                if (ReusableMethods.Lists.IsNullOrEmpty(ref m_SeriesTrainingExamplesNode.TrainingExamplesVector))
                {
                ...
                else
                {
                    ...
                    for (int i = 0; i < m_SeriesTrainingExamplesNode.TrainingExamplesVector.Count; i++)
                    {
                        EditorGUILayout.LabelField("Training Example " + (i+1));

                        EditorGUI.indentLevel++;

                        var inputFeatures = m_SeriesTrainingExamplesNode.TrainingExamplesVector[i].Inputs;
                        var outputFeatures = m_SeriesTrainingExamplesNode.TrainingExamplesVector[i].Outputs;
